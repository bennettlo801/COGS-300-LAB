...
void moveMotor(const char* direction, unsigned long duration) {
  ...
  // NOTE: LEFT and RIGHT blocks have been SWAPPED here
    ...
    analogWrite(motorA, SPEED);
    digitalWrite(motorApin1, HIGH);   // left wheel forward
    digitalWrite(motorApin2, LOW);

    analogWrite(motorB, SPEED);
    digitalWrite(motorBpin1, LOW);    // right wheel backward
    ...
    analogWrite(motorA, SPEED);
    digitalWrite(motorApin1, LOW);    // left wheel backward
    digitalWrite(motorApin2, HIGH);

    analogWrite(motorB, SPEED);
    digitalWrite(motorBpin1, HIGH);   // right wheel forward
    digitalWrite(motorBpin2, LOW);
  }
}
