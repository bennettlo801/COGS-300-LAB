...
void followWall() {
  ...
  // --- Wall lost: immediate left turn ---
    ...
    stop();
    delay(50);
    Serial.println("Wall lost — performing timed 90° turn...");

    left(TURN_SPEED);       // right wheel forward, left wheel frozen
    delay(2500);             // calibrate this for your robot to turn ~90°
    stop();
    delay(150);

    // move forward slightly to allow wall detection
    forward(BASE_SPEED);
    delay(1200);  // tune this based on your robot
    stop();
}
