...
// gradual left turn: right wheel full speed, left wheel slowed
void gradualLeft(int baseSpeed, int leftWheelFactor) {
  // leftWheelFactor = 0–100 (% of baseSpeed)
  int leftSpeed  = baseSpeed * leftWheelFactor / 100;
...
void followWall() {
  ...
  // --- Wall lost: immediate left turn ---
    ...
    stop();
    delay(50);
    Serial.println("Wall lost — performing gradual turn...");

    // Gradually turn left until a certain distance forward
    unsigned long turnStart = millis();
    while (millis() - turnStart < 1000) {  // 1 second turn
      gradualLeft(BASE_SPEED, 20);  // left wheel at 40% speed
      delay(50);
    }
    ...
    // move forward slightly to reacquire wall
    forward(BASE_SPEED);
    delay(300);
    stop();
}
